...
function Index({route}) {
    ...
    return (
        <ScreenMask>
            <ScrollView
                ref={scrollRef}
                onScrollEndDrag={(event) => +event.nativeEvent.contentOffset.x > +data.length*290? onPressTouch(0) : +event.nativeEvent.contentOffset.x === 0 ? onPressTouch(data.length * 302) : null}
                horizontal={true}
