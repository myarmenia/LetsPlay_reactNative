...
function Index(props) {
  ...
  const LinkItem = ({ item }) => (
    <TouchableOpacity
      onPress={() =>
        item.id !== 2 ? navigation.navigate(item.navigationTo) : console.log('История')
      }
      style={
        item.id === 3
          ? {
              ...styles.linkBlock,
              paddingVertical: RH(27),
              borderBottomWidth: 1,
              borderTopWidth: 1,
            }
          : {
  ...
  return (
    <ScreenMask style={{ paddingHorizontal: 0 }}>
      ...
      <View style={style.priceBlock}>
        <Text style={style.balanceText}>Ваш баланс:</Text>
        <Text style={style.price}>0 руб</Text>
      </View>
      <FlatList data={list} renderItem={renderItem} keyExtractor={(item) => item.id} />
      <View style={style.buttonBlock}>
        <Button size={{ width: 281, height: 48 }} label={'Пополнить баланс'} />
      </View>
    </ScreenMask>
  )
}
