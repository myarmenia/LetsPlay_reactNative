...
const Elias = ({navigation}) => {
    ...
    return (
        <ScreenMask>
            <DraxProvider>
                <View style={styles.container}>
                    ...
                    <DraxView
                        ...
                        onReceiveDragDrop={(event) => {
                            if (event.dragged.payload.id && comOne.length<5 && !comOne.filter((item, i) => item.id === event.dragged.payload?.id).length) {
                                setComOne([
                                    ...comOne,
                                    event.dragged.payload
                                ])
                                const filterDataComTwo = comTwo.filter((item, i) => item.id !== event.dragged.payload.id)
                                setComTwo(filterDataComTwo)
                                const filterDataList = list.filter((item, i) => item.id !== event.dragged.payload.id)
                ...
                <View style={styles.container}>
                    ...
                    <DraxView
                        ...
                        onReceiveDragDrop={(event) => {
                            if (event.dragged.payload.id &&  comTwo.length<5 && !comTwo.filter((item, i) => item.id === event.dragged.payload?.id).length) {
                                setComTwo([
                                    ...comTwo,
                                    event.dragged.payload
                                ])
                                const filterDataComOne = comOne.filter((item, i) => item.id !== event.dragged.payload.id)
                ...
                <View style={styles.container}>
                    <DraxView
                        ...
                        onReceiveDragDrop={(event) => {
                            if (event.dragged.payload.id && !list.filter((item, i) => item.id === event.dragged.payload?.id).length ) {
                                setList([
                                    event.dragged.payload,
                                    ...list,
                                ])
            ...
            <View style={{marginLeft: 'auto', alignItems:'center', marginTop: 20, marginRight: 'auto'}}>
                <Button onPress={() => navigation.navigate('EliasAllocatePlayersInfo', {comTwo, comOne})}
