...
export const signUpFirstStep = (user) => (dispatch) => {
    dispatch(pending());
    axiosInstance.post('api/auth/signup/first_step', JSON.stringify(user)).then(
        (response) => {
            dispatch(firstStepFulfilled(response.data.expired_token))
        })
        .catch((err) => {
            return dispatch(firstStepRejected({
                id: new Date().getTime(),
...
export const signUpSecondStep = (data) => (dispatch) => {
    dispatch(pending());
    axiosInstance.post('api/auth/signup/second_step', JSON.stringify(data)).then(
        (response) => {
            if(+response.data.statusCode>=400){
                dispatch(secondStepRejected({
                    id: new Date().getTime(),
        ...
        .catch((err) => {
            return dispatch(secondStepRejected({
                id: new Date().getTime(),
                text: 'Неверный код с почты повторите заново',
            }))
        })
