...
const DateTime = (props) => {
    const {type , day , data , setData , gameDayDate , errorText} = props
    const currentDate = new Date()
    const minimumDate = currentDate.setDate(currentDate.getDate() + 1)
    const [date, setDate] = useState("ДД/ММ/ГГГГ")
    const lastDate = new Date(gameDayDate).setDate((new Date(gameDayDate)).getDate() - 1) || new Date('2099-12-31')
    ...
    return (
        <View style={{flexDirection: 'column'}}>
            <TouchableOpacity style={type === 'time'? style.timeButton : style.dateButton } onPress={() => {
                setOpen(true)
            }}>
                {type === 'date' ? <DateSvg style={style.dateSvg}/> : <TimeSvg style={style.dateSvg}/>}
                <Text style={style.dateButtonText}>{
                    type === 'date' ?  (date === "ДД/ММ/ГГГГ" ? date : moment(date).format('DD/MM/YY')): (date === "ДД/ММ/ГГГГ" ?moment(new Date(minimumDate)).format('HH:mm') :moment(date).format('HH:mm'))}
                </Text>
            </TouchableOpacity>
            <DatePicker
                modal
                minimumDate={type === 'date' ? (day === 'gameDay' ? new Date(minimumDate) : new Date()) : new Date(1900, 1, 1)}
                maximumDate={type === 'date' ? new Date(lastDate) : new Date(2099, 31, 12)}
                mode={type === 'date' ? 'date' : 'time'}
                open={open}
                date={date === "ДД/ММ/ГГГГ" ? new Date(minimumDate) : (type === 'date' ? date : new Date(minimumDate))}
