  ...
  return (
    <View style={style.bg}>
      <View style={isMax ? style.imgMaxBlock : style.imgBlock}>
        {user.image ? (
          <Image style={style.image} source={{ uri: user.image }} />
        ) : (
          <UserDefault isMax={isMax} />
        )}
      </View>
      <View style={style.nameBlock}>
        <Text style={isMax ? style.nameMax : style.name}>{user.lName ? user.lName : 'Имя'}</Text>
        <Text style={isMax ? style.nameMax : style.name}>
      ...
      <View
        style={{
          ...style.titleBigBloc,
          marginTop: RW(isMax ? 10 : 3),
          width: RW(isMax ? 210 : 50),
        }}
      >
