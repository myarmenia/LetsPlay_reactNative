  ...
  return (
      list.map((ev) => <TouchableOpacity style={{...style.radioBlock, marginLeft: margin}} onPress={() => {handleChange(ev)}} key={ev.id}>
        <LinearGradient
            ...
            style={{ borderWidth: 1, alignItems: 'center', justifyContent: 'center', width: RW(16) , height: RH(16), borderRadius: RW(16)}}
        >
          { (checked && ev.checked) || value === ev.id ? <LinearGradient

          colors={['#7DCE8A', '#4D7CFE']}
              start={{x: 0.0, y: 0.0}} end={{x: 1.0, y: 0.0}}
              style={{ borderWidth: 1, alignItems: 'center', justifyContent: 'center', width: RW(10) , height: RH(10), borderRadius: RW(10)}}
          >

          </LinearGradient>: <View style={{ borderWidth: 1, alignItems: 'center', backgroundColor: LIGHT_LABEL, justifyContent: 'center', width: RW(10) , height: RH(10), borderRadius: RW(10)}}></View>}
        </LinearGradient>
